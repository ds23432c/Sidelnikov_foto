...
window.initProfileMap = function(lat, lng) {
  ...
  const map = L.map('profile-map-picker', { attributionControl: false }).setView([initLat, initLng], lat ? 10 : 5);
...
window.initCreatorsMap = function() {
  ...
  if (!mapEl) return;
  const map = L.map('map', { attributionControl: false }).setView([60, 55], 4);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
